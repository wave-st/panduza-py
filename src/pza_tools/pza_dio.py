...
Should accept one argument (the object to be represented), and return a string."""
    ...
    def custom_print_cell(self, actual_cell, cell_display_value):
        ...
        if actual_cell.grid_current_value_index != -1:
            ...
            if column_indexer == 1:
                if cell_display_value == "INPUT":
                    actual_cell.color = 'WARNING'
                else:   
                    actual_cell.color = 'WARNING'

            if column_indexer == 2:
                if cell_display_value == "LOW":
...
class MainForm(npyscreen.FormBaseNew):
    def create(self):
        self.grid = self.add(ComplexColumnGrid, columns_widths=[ 0.5, 0.1, 0.2, 0.1 ])
        self.grid.values = [ ]
...
class DioController(npyscreen.NPSAppManaged):
    ...
    def onStart(self):
        ...
        self.main_form = MainForm(name='Panduza DioController')
    ...
    def _on_message(self, client, userdata, msg):
        ...
        if msg.topic.endswith("info"):
            ...
            if type == 'io':
                ...
                if idx == -1:
                    self.main_form.grid.values.append([ base_topic, "INPUT", "LOW", int(now - self.t0) ])
                    client.subscribe(base_topic + "/atts/value")
                    client.subscribe(base_topic + "/atts/direction")
                else:
                    self.main_form.grid.values[idx][3] = str( int(now - self.t0) )
        
        elif msg.topic.endswith("atts/value"):
            
            info = json.loads( msg.payload.decode("utf-8") )
            if isinstance(info, str):
                info = json.loads(info)
            base_topic = msg.topic[:-11]
            ...
            if idx != -1:
                ...
                self.main_form.grid.values[idx][2] = text

        elif msg.topic.endswith("atts/direction"):
            pass
    ...
    def interface_grid_index(self, base_topic):
        ...
        """
        for i in range(len(self.main_form.grid.values)):
            if self.main_form.grid.values[i][0] == base_topic:
                return i
        return -1
